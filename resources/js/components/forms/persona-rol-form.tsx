...
type PersonaRolFormData = PersonaRolFullType & {
  pais_texto?: string | null;
  provincia_texto?: string | null;
  localidad_texto?: string | null;
};

type PersonaRolFormProps = {
    personaRol?: PersonaRolFullType;
    personas: DropdownOption[];
    roles: DropdownOption[];
    errors?: Record<string, string>;
    onSuccess?: (personaRol: PersonaRolFullType) => void;
    onSubmit?: (data: PersonaRolFormData) => Promise<void> | void;
    isModal?: boolean;
};
...
export default function PersonaRolForm({
    ...
    const validPersonas = Array.isArray(personas) ? personas : [];
    ...
    const { data, setData, processing, reset } = useForm<PersonaRolFormData>({
        id: personaRol?.id ?? 0,
        activo: personaRol?.activo ?? true,
        rol: personaRol?.rol ?? null,
        persona_id: personaRol?.persona_id ?? null,
        persona: {
            id: personaRol?.persona?.id ?? 0,
            nombre: personaRol?.persona?.nombre ?? '',
            apellido: personaRol?.persona?.apellido ?? '',
            dni: personaRol?.persona?.dni ?? null,
            activo: personaRol?.persona?.activo ?? true,
            pais_id: personaRol?.persona?.pais_id ?? null,
            provincia_id: personaRol?.persona?.provincia_id ?? null,
            localidad_id: personaRol?.persona?.localidad_id ?? null,
            domicilio: personaRol?.persona?.domicilio ?? null,
            telefono: personaRol?.persona?.telefono ?? null,
            email: personaRol?.persona?.email ?? null,
        },
        pais_texto: null,
